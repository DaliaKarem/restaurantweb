...
async function getAllProducts() {
    try {
        const response = await fetch('/api/v1/Products'); // Assuming your server is running on the same host
        const productsData = await response.json();
        console.log(productsData);
        // Process the received products
        // eslint-disable-next-line no-undef
        
        //const productListContainer = document.getElementById('productDetails');
        //productListContainer.innerHTML = ''; // Clear existing content

        // Iterate through products and append HTML to productListContainer
